    ...
    int depth)
{
    ...
    if (auto payload = trace(orig, dir, scene.get_objects()); payload)
    {
        ...
        case REFLECTION_AND_REFRACTION:
        {
            ...
            // fix: full reflection when n1 > n2
            Vector3f refractionColor(0.0, 0.0, 0.0);
            if (dotProduct(refractionDirection, refractionDirection) != 0)
            {
                refractionColor = castRay(refractionRayOrig, refractionDirection, scene, depth + 1);
            }
        ...
        default:
        {
            ...
            for (auto &light : scene.get_lights())
            {
                ...
                // TODO: Specular reflection shadow ray?
...
void Renderer::Render(const Scene &scene)
{
    ...
    for (int j = 0; j < scene.height; ++j)
    {
        for (int i = 0; i < scene.width; ++i)
        {
            ...
            float plane_width = 2 * scale * imageAspectRatio;
            float plane_height = 2 * scale;
            y = scene.height - j - 1 + 0.5;
            x = i + 0.5;
            y = (y / scene.height) * plane_height - scale;
            x = (x / scene.width) * plane_width - scale * imageAspectRatio;
